...
//章节类
class ChapterAction extends LoadingAction {
    ...
    public function index($book, $num) {
       ...
       // print_r($this->content);exit();
        ...
        if (!is_array($this->content)) {
            //生成内容缓存
            $con = $this->bookcontent->where(array('fu_book' => $this->books[fu_book], 'num' => $num))->field('content_id,num,title,number,the_price')->find();
            if (is_array($con)) {
